...
<body aria-busy="true">
  <?php
    ...
    if(isset($_POST['simpan'])){
      ...
      $ktpfolder = "gambarPenyewa/ktp/".$ktpname;
      $selfiename = $_FILES['ktpFotoSelfie']['name'];
      $selfietmp = $_FILES['ktpFotoSelfie']['tmp_name'];
      $selfiefolder = "gambarPenyewa/selfieKtp/".$selfiename;
      move_uploaded_file($ktptmp,$ktpfolder);
      move_uploaded_file($selfietmp,$selfiefolder);

      $sql = "INSERT INTO penyewa SET nama_penyewa='$nama',email_penyewa='$email',
              notelp_penyewa='$notelp',noktp_penyewa='$noktp',gamktp_penyewa ='$ktpname',
              gamselfie_penyewa='$selfiename'";
      $result = mysqli_query($mysqli,$sql);
      if($result){
        echo "Data Berhasil Dimasukkan";
        $_SESSION["nama"]=$nama;
        header("location:register2_penyewa.php");
      }
    }
  ?>
  <!-- Main navigation -->
  <header>  
    <!-- Full Page Intro -->
    <div class="view" style="background-image: url('./gambarPenyewa/tampilan/10-edit.jpg'); background-repeat: no-repeat; background-size: cover; background-position: center center; height:900px;">
